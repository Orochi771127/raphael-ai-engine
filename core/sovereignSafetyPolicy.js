...
const HIGH_RISK_ROUTES = [
  ...
  ['acute_medical', [
    ...
    // 仍要求同時出現藥物名詞，避免「我吃了太多飯」這類一般句誤判。
    /(?:吞|吃|服用|服下|喝).{0,6}(?:太多|很多|大量|過量|过量|一整罐|一整瓶|一整包|整罐|整瓶|整包|一大把).{0,6}(?:藥|药|藥丸|药丸|安眠藥|安眠药|止痛藥|止痛药|毒物)/u,
    /(?:一整罐|一整瓶|一整包|整罐|整瓶|整包|一大把|大量|過量|过量).{0,10}(?:藥|药|安眠藥|安眠药|止痛藥|止痛药).{0,6}(?:吃完|喝完|吞完|吃光|吞下|服下|吃下)/u,
    /(?:胸痛|胸口劇痛|胸口剧痛|呼吸困難|呼吸困难|喘不過氣|喘不过气|大量出血|失去意識|失去意识|昏迷|抽搐)/u,
    /(?:吐血|咳血|嚴重出血|严重出血)/u,
    /(?:快昏倒|快暈倒|快晕倒|要昏倒|昏倒).{0,12}(?:心跳很亂|心跳很乱|心悸|喘不過氣|喘不过气|冒冷汗|站不住)/u,
    /(?:overdose|overdosed|overdosing|took too many pills|took a (?:whole|entire) bottle|swallowed a (?:whole|entire) bottle|chest pain|cannot breathe|can't breathe|coughing up blood)/iu,
  ]],
  ['active_abuse', [
  ...
  ['self_or_other_harm', [
    ...
    // 自傷的動作描述：原本只收固定詞「割腕」，玩家寫「割了自己的手腕」
    // 或「拿刀劃自己」都會整段漏接。
    /(?:割|劃|划|刺)(?:了|傷|伤)?(?:自己的?|我的)?(?:手腕|手臂|大腿|身體|身体)/u,
    /(?:拿|用)(?:刀|美工刀|刀片|剪刀).{0,8}(?:割|劃|划|刺).{0,6}(?:自己|我的)/u,
    /(?:suicide|suicidal|kill myself|killing myself|hurt myself|harm myself|hurting myself|kill them|hurt someone|end (?:my|this) life|ending (?:my|this) life|cut(?:ting)? my (?:wrist|wrists|arm|arms|thigh|thighs)|slit(?:ting)? my (?:wrist|wrists))/iu,
  ]],
];
