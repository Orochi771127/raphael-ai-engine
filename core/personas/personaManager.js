...
export function generatePersonaReply(persona, intents, context, playerProfile = {}) {
  ...
  for (const intent of intents) {
    ...
    if (context && context.lastIntent === intent) {
      ...
      if (persona.dialogue[continueKey]) {
        rawReply = persona.dialogue[continueKey];
        break;
      }
    }
    ...
    if (persona.dialogue[intent]) {
      rawReply = persona.dialogue[intent];
      break;
    }
  }
  ...
  if (playerName) {
    return rawReply.replace(/\{playerName\}/g, playerName);
  }
  return rawReply.replace(/\{playerName\}/g, '你');
}
